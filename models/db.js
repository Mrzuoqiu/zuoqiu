...
var DbSet = {
    ...
    uploadLogo:function (User,Article,Comment,req,callback) {
        User.findByIdAndUpdate(req.session.user._id,{logo:'/upload/users/logos/' + req.file.filename},function (err) {
            if(err) {
                return callback(err);
            }
            Article.update({author:req.session.user.username},{authorLogo:'/upload/users/logos/'+req.file.filename},{multi:true},function (err) {
                if(err) {
                    return callback(err);
                }
                Comment.update({commenter:req.session.user.username},{commenterLogo:'/upload/users/logos/' + req.file.filename},{multi:true},function (err) {
                    if(err) {
